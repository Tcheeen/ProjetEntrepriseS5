...
<body>
            ...
            <div class="col-sm-12 col-lg-10 col-xl-9 col-xxl-7 bg-white shadow-lg" style="border-radius: 5px;">
                <div class="p-5">
                    <div class="text-center">
                        ...
                        <h4 class="text-dark mb-4" style="font-size: 20px;margin-top: 12px;">Réponse du candidat n°{{ $idcandidat }}</h4>
                    </div>
                    <form action="{{url('/eval_note_test')}}" method="post" class="user" style="text-align: center;">
                    {{ csrf_field() }}
                        ...
                        <div class="row mb-3">
                            <div class="row">
                                ...
                                @foreach($liste_quest as $rows)
                                    <p style="text-align: center;">Question <?php echo $i;?> :&nbsp;<input type="checkbox" name="vrai[]" value="1">&nbsp;Vrai&nbsp;&nbsp;<input type="checkbox" name="faux[]" value="0">&nbsp;Faux</p>
                                    <?php $i++; ?>
                                @endforeach
                                </div>
                            </div>
                        ...
                        </div><button class="btn btn-primary" type="submit" style="text-align: center;background: var(--bs-gray);border-style: none;">Valider</button>
                        <hr>
                    </form>
                </div>
            </div>
    ...
    <script src="<?php echo asset('assets/eval_test/bootstrap/js/bootstrap.min.js');?>"></script>
    <script src="<?php echo asset('assets/eval_test/js/bs-init.js');?>"></script>
...
<html>
    <head><title>Validation de resultats</title></head>
    <body>
     <h1>Reponse du {{ $idcandidat }}</h1>
     <table border="1">
         <form action="{{url('/eval_note_test')}}" method="post">
             {{ csrf_field() }}
             <input type="hidden" name="idcandidat" value="{{ $idcandidat }}" >
             <input type="hidden" name="idannonce" value="{{ $idannonce }}" >
             @foreach($liste_quest as $rows)
            ...
            <p>Question<?php echo $i; ?>:
                {{ $rows->question }}</p>
        <input type="checkbox" name="vrai[]" value="1">
        <label for="homme">Vrai</label>
        <input type="checkbox" name="faux[]" value="0">
        <label for="femme">Faux</label>
            <?php $i++; ?>
            @endforeach
            </tr>
        </table>
    ...
    <p><button><a href="{{url('/retour')}}/{{$idannonce}}">Retour</a></button></p>
